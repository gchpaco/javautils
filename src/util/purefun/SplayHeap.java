...
public abstract class SplayHeap<T extends Comparable<? super T>> implements
        Heap<T> {
    // I give you the finger, Java generics!
    protected static <T extends Comparable<? super T>> Pair<SplayHeap<T>, SplayHeap<T>> make(
            SplayHeap<T> l, SplayHeap<T> r) {
    ...
    protected static class Node<T extends Comparable<? super T>> extends
        ...
        protected Pair<SplayHeap<T>, SplayHeap<T>> partition(T pivot) {
            ...
            if (element.compareTo(pivot) <= 0) {
                if (right.isEmpty())
                    return make(this, right);
                Node<T> r = (Node<T>) right;
                if (r.element.compareTo(pivot) <= 0) {
                    Pair<SplayHeap<T>, SplayHeap<T>> p = r.right
                            .partition(pivot);
                    return make(new Node<T>(new Node<T>(left, element, r.left),
                            r.element, p.first), p.second);
                }
                Pair<SplayHeap<T>, SplayHeap<T>> p = r.left
                        .partition(pivot);
                return make(new Node<T>(left, element, p.first),
                        new Node<T>(p.second, r.element, r.right));
            }
            if (left.isEmpty())
                return make(left, this);
            Node<T> l = (Node<T>) left;
            if (l.element.compareTo(pivot) <= 0) {
                Pair<SplayHeap<T>, SplayHeap<T>> p = l.right
                        .partition(pivot);
                return make(new Node<T>(l.left, l.element, p.first),
                        new Node<T>(p.second, element, right));
            }
            Pair<SplayHeap<T>, SplayHeap<T>> p = l.left
                    .partition(pivot);
            return make(p.first, new Node<T>(p.second, l.element,
                    new Node<T>(l.right, element, right)));
        }

        public T findMin() throws EmptyException {
            return left.isEmpty() ? element : left.findMin();
        }
        ...
        public SplayHeap<T> deleteMin() throws EmptyException {
            ...
            Node<T> l = (Node<T>) left;
            if (l.left.isEmpty())
                return new Node<T>(l.right, element, right);
            return new Node<T>(l.left.deleteMin(), l.element, new Node<T>(
        ...
        public SplayHeap<T> merge(Heap<T> h) {
            ...
            return new Node<T>(p.first.merge(left), element, p.second
                    .merge(right));
        }
    }
