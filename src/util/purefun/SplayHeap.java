                                                                 ...
                                                                 Heap<T>
{
      ...
      SplayHeap<T>
  {
    ...
    @SuppressWarnings ("unchecked")
    @Override
    protected Pair<SplayHeap<T>, SplayHeap<T>> partition (
    ...
    protected Pair<SplayHeap<T>, SplayHeap<T>> partition (T pivot)
      {
        ...
        if (element.compareTo (pivot) <= 0)
          {
            ...
            if (r.element.compareTo (pivot) <= 0)
              {
                Pair<SplayHeap<T>, SplayHeap<T>> p = r.right.partition (pivot);
                return make (new Node<T> (new Node<T> (left, element, r.left),
                                          r.element, p.first), p.second);
              }
            Pair<SplayHeap<T>, SplayHeap<T>> p = r.left.partition (pivot);
        ...
        if (l.element.compareTo (pivot) <= 0)
          {
            Pair<SplayHeap<T>, SplayHeap<T>> p = l.right.partition (pivot);
            return make (new Node<T> (l.left, l.element, p.first),
                         new Node<T> (p.second, element, right));
          }
        Pair<SplayHeap<T>, SplayHeap<T>> p = l.left.partition (pivot);
        return make (p.first,
                     new Node<T> (p.second, l.element, new Node<T> (l.right,
                                                                    element,
                                                                    right)));
      }
    ...
    public SplayHeap<T> deleteMin () throws EmptyException
      {
        ...
        return new Node<T> (l.left.deleteMin (), l.element,
