...
public class QueueTest {
	...
	Queue<Integer> queue = BatchedQueue.empty().snoc(6).snoc(5)
	    .snoc(2).snoc(7);
	try {
	    assertEquals((Integer)6, queue.head());
	    assertEquals((Integer)5, queue.tail().head());
	    assertEquals((Integer)2, queue.tail().tail().head());
	    assertEquals((Integer)7, queue.tail().tail().tail().head());
